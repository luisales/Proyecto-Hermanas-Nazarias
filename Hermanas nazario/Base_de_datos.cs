...
namespace Hermanas_nazario
{
    public class Base_de_datos
    {
        ...
        public static SqlConnection Conectar()
        {
            ...
            //SqlConnection con = new SqlConnection("Data Source=OMAR\\SQLEXPRESS;Initial Catalog=HermanasNazario;Integrated Security=True");
            return con;
        ...
        public static int ValidarExp(string id)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                ...
                if (registro.Read())
                {
                    ...
                    Base_de_datos.estadoE = registro["Estado"].ToString();
        ...
        public void BuscarMedNom(string Nom)
        {
            ...
            {
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select a.Codigo 'Código Medicamento', a.Nombre 'Nombre Medicamento',a.Cantidad 'Cantidad',a.Precio 'Precio', a.Descripcion 'Descripcion Medicamento', b.Nombre 'Medida', [dbo].[busqueda](a.Codigo_empleado_ingreso) 'Empleado de ingreso', a.Fecha_ingreso, [dbo].[busqueda](a.Codigo_empleado_modificacion) 'Empleado de modificacion' , a.Fecha_modificacion from [dbo].[Medicamento] a inner join Medida b on a.Codigo_medida=b.[Codigo] where a.Nombre LIKE " + "'" + Nom + "%'", con);
                da.SelectCommand.CommandType = CommandType.Text;
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        public int BuscarReceta(string id)
        {
            ...
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(" select b.[Codigo] 'Codigo Medicamento', b.[Nombre] 'Nombre Medicamento', a.[Cantidad], (b.[Precio]) 'Precio Medicamento', (a.[Cantidad] *b.[Precio]) 'Total' from [dbo].[Receta] a  inner join [dbo].[Medicamento] b on a.Codigo_medicamento = b.Codigo where a.Codigo_cita LIKE " + "'" + id + "'", con);
            da.SelectCommand.CommandType = CommandType.Text;
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        public static float total_medicamentos(string id)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("select sum(round((a.[Cantidad] *b.[Precio]),1,0)) as total from [dbo].[Receta] a  inner join [dbo].[Medicamento] b on a.Codigo_medicamento = b.Codigo where a.Codigo_cita LIKE " + "'" + id + "'", con);
            SqlDataReader registro = cmd.ExecuteReader();
        ...
        public static void Actualizar_empleado(int codigo_empleado, string nombre1, string nombre2, string apellido1, string apellido2, string correo_empleado, string id_empleado, string sexo, string tel_empleado, int rol, int cargo)
        {
            SqlConnection con;
            con = Base_de_datos.Conectar();
        ...
        public void BuscarEE(string nom, String ape)
        {
            ...
            {
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select a.Codigo 'Codigo empleado', a.Primer_nombre 'Primer nombre', a.Segundo_nombre 'Segundo nombre',a.Primer_apellido 'Primer Apellido',a.Segundo_apellido 'Segundo Apellido', a.Numero_identidad 'Identidad de empleado', a.Sexo 'Sexo de empleado', a.Correo_empleado 'Correo de empleado', a.Telefono 'Telefono de empleado',e.Nombre 'Puesto', d.Nombre 'Rol', a.Estado, a.Fecha_modificacion 'Fecha ingreso',[dbo].[busqueda](a.Codigo_empleado_ingreso) 'Empleado ingreso',a.Fecha_modificacion 'Fecha modificacion', [dbo].[busqueda](a.Codigo_empleado_modificacion) 'Empleado modificacion' from [dbo].Empleado a inner join [dbo].[Rol] d on a.Codigo_rol = d.Codigo inner join Puesto e on e.Codigo=a.Codigo_puesto where a.Primer_nombre LIKE " + "'" + nombre + "%'" + " AND a.Primer_apellido LIKE " + "'" + ape + "%'", con);
                da.SelectCommand.CommandType = CommandType.Text;
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        public static int Buscar_codigo_puesto()
        {
            int x;
            SqlConnection con;
            con = Conectar();
            con.Open();
            SqlCommand cmd = new SqlCommand("select Codigo from [dbo].[Puesto] where Nombre='" + cargo_empleado + "'", con);
            SqlDataReader Perm = cmd.ExecuteReader();
            if (Perm.Read())
            {
        ...
        public static int Buscar_codigo_rol()
        {
            int x;
            SqlConnection con;
            con = Conectar();
            con.Open();
            SqlCommand cmd = new SqlCommand("select Codigo from [dbo].[Rol] where Nombre='" + codigo_rol + "'", con);
            SqlDataReader Perm = cmd.ExecuteReader();
            if (Perm.Read())
            {
                return Convert.ToInt32(Perm["Codigo"]);
        ...
        public void BuscarRopaNom1(string Nom)
        {
            ...
            {
                con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select a.Codigo  'Código Prenda', a.descripcion  'Nombre Prenda', a.Existencia, [dbo].[busqueda](a.Codigo_empleado_ingreso) 'Empleado ingreso', [dbo].[busqueda](a.Codigo_empleado_modificacion) 'Empleado Modificacion', a.Fecha_ingreso, a.Fecha_modificacion, d.Nombre 'Categoria', a.Estado from [dbo].[Ropa] a inner join [dbo].[Categoria] d on a.Codigo_categoria=d.Codigo where a.Descripcion LIKE " + "'" + Nom + "%'", con);
                da.SelectCommand.CommandType = CommandType.Text;
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        public static int validarRopaMod(string id, string nombre)
        {
            SqlConnection con;
            con = Base_de_datos.Conectar();

            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * from Ropa WHERE Descripcion=@id and Codigo != @nombre", con);
            cmd.Parameters.AddWithValue("id", id);
            cmd.Parameters.AddWithValue("nombre", nombre);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
        ...
        public static int validarNomPuestoMod(string id, string nom)
        {
            SqlConnection con;
            con = Base_de_datos.Conectar();

            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT * from Puesto WHERE Codigo!=@id and Nombre != @nom", con);
            cmd.Parameters.AddWithValue("id", id);
            cmd.Parameters.AddWithValue("nom", nom);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
        ...
        public static Boolean validarContraseña(String email)
        {
            String expresion;
            expresion = "^(?!.*(.)\\1{3})((?=.*[\\d])(?=.*[a-z])(?=.*[A-Z])|(?=.*[a-z])(?=.*[A-Z])(?=.*[^\\w\\d\\s])|(?=.*[\\d])(?=.*[A-Z])(?=.*[^\\w\\d\\s])|(?=.*[\\d])(?=.*[a-z])(?=.*[^\\w\\d\\s])).{7,30}$";
            if (Regex.IsMatch(email, expresion))
            {
                if (Regex.Replace(email, expresion, String.Empty).Length == 0)
                {
